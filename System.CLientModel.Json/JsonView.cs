...
public readonly struct JsonView
{
    ...
    public void Set(ReadOnlySpan<byte> name, ReadOnlySpan<byte> json) => _model.Set(name, json);
    ...
    public string GetString(ReadOnlySpan<byte> name)
    {
        ...
        if (slashIndex > 0)
        {
            ...
            ReadOnlySpan<byte> propertyJson = _model.Get(propertyName);
        ...
        if (_path.Length > 0)
        {
            ...
            ReadOnlySpan<byte> json = _model.Get(_path);
        ...
        else
        {
            ...
            ReadOnlySpan<byte> valueData = _model.Get(name);
    ...
    // TODO: Can we eliminate this? It allocates
